...
def is_supported_knowledge_file(
    path: Path,
) -> bool:
    if not isinstance(
        path,
    ...
    return (
        ...
        in SUPPORTED_KNOWLEDGE_FILE_SUFFIXES
    )
...
def iter_knowledge_files(
    ...
    if path.is_file():
        if is_supported_knowledge_file(
            path,
        ):
            return [
                path,
...
def load_rag_store_from_path(
    *,
    path: str | Path,
    max_chars: int = DEFAULT_RAG_CHUNK_MAX_CHARS,
    overlap_chars: int = DEFAULT_RAG_CHUNK_OVERLAP_CHARS,
) -> InMemoryRagStore:
    knowledge_path = Path(
        path,
    )
    ...
    for file_path in iter_knowledge_files(
        knowledge_path,
    ):
